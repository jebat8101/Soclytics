...
from platforms.x.about_sb import main as scrape_about
from platforms.x.constants import (
    BASE_DIR, COOKIE_FILE, DB_FILE, DEPTH_LIMITS, FACE_DIR, ICONS_DIR,
    PIPELINE_STEPS,
)
from platforms.x.db import (
    compute_frequency, extract_top7, get_x_engagement, get_x_timeline, import_all,
)
from platforms.x.posts_sb import main as scrape_posts
...
register_report_routes(x_bp, DB_FILE, 'x')


@x_bp.route('/')
def home():
    cookie_status = _check_cookie_status_fast()
    return render_template('x/index.html', cookie_status=cookie_status)
...
def analysis():
    ...
    if not profile_id:
        return redirect(url_for('x.home'))
    profile = get_profile_summary(DB_FILE, profile_id)
    if not profile:
        return redirect(url_for('x.home'))
    return render_template('x/analysis.html', profile=profile, profile_id=profile_id)


@x_bp.route('/api/import-cookies', methods=['POST'])
def import_cookies():
    ...
    try:
        ...
        if not cookies_have_any_domain(cookies, list(X_COOKIE_DOMAINS)):
            return jsonify({
                'ok': False,
                'error': 'No X cookies found - export cookies from x.com while logged in',
            })

        x_cookies = filter_cookies_for_any_domain(cookies, list(X_COOKIE_DOMAINS))
        names = {c.get('name') for c in x_cookies}
        if 'auth_token' not in names:
            return jsonify({
                'ok': False,
                'error': 'Missing auth_token cookie — log into x.com before exporting',
            })
    ...
    try:
        ...
        if not cookies_have_any_domain(cookies, list(X_COOKIE_DOMAINS)):
            return {
                ...
                'count': len(cookies),
                'cookies': cookies,
                'error': 'Cookie file has no X / Twitter domain cookies',
            }
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT
                ...
                (SELECT COALESCE(SUM(view_count), 0) FROM text_posts WHERE profile_id = p.id) AS view_count,
...
@x_bp.route('/api/investigations/<int:profile_id>', methods=['DELETE'])
...
@x_bp.route('/api/start-pipeline', methods=['POST'])
def start_pipeline():
    ...
    try:
        profile_url = normalize_x_target(profile_url)
...
@x_bp.route('/api/pipeline-status')
def pipeline_status():
    return jsonify({
        'ok': True,
...
def _run_pipeline(profile_url, depth):
    ...
    try:
        ...
        try:
            import_all(
                about_json=os.path.join(BASE_DIR, 'x_about.json'),
                posts_json=os.path.join(BASE_DIR, 'x_posts.json'),
                db_file=DB_FILE,
...
@x_bp.route('/api/profile-summary/<int:profile_id>')
def api_profile_summary(profile_id):
    try:
        data = get_profile_summary(DB_FILE, profile_id)
        data['engagement'] = get_x_engagement(DB_FILE, profile_id)
        return jsonify({'ok': True, 'data': data})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500


@x_bp.route('/api/all-interactors/<int:profile_id>')
...
@x_bp.route('/api/top7/<int:profile_id>')
...
@x_bp.route('/api/graph-data/<int:profile_id>')
...
@x_bp.route('/api/cocomment-graph/<int:profile_id>')
...
@x_bp.route('/api/timeline/<int:profile_id>')
def api_timeline(profile_id):
    try:
        return jsonify({'ok': True, 'data': get_x_timeline(DB_FILE, profile_id)})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500
...
def api_post_type_counts(profile_id):
    try:
        eng = get_x_engagement(DB_FILE, profile_id)
        return jsonify({
            'ok': True,
            'data': {
                'posts': {
                ...
                'interactions': {
                    ...
                    'reel': eng['repost'],
                    'text': eng['reply'],
                    'total': eng['reply'] + eng['repost'] + eng['like'],
                },
                'engagement': eng,
            },
        })
...
@x_bp.route('/api/face-clusters/<int:profile_id>')
...
@x_bp.route('/api/face-cluster/<int:cluster_id>/members')
...
@x_bp.route('/api/photo-posts/<int:profile_id>')
...
@x_bp.route('/api/text-posts/<int:profile_id>')
...
@x_bp.route('/api/reel-posts/<int:profile_id>')
...
@x_bp.route('/face-image/<path:filepath>')
