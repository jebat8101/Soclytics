"""Instagram SQLite schema + fixture import.

Table names match Facebook so core.scoring / core.face work unchanged.
Feed posts map into photo_posts; reels into reel_posts.
"""
import sqlite3
import json
import os
...
-- Feed posts stored as photo_posts (FB-compatible for scoring/face)
...
-- Text posts (unused by IG import; kept for schema parity)
...
-- Text post comments (schema parity)
...
-- Top 7 most frequent interactors
...
def import_about(about_json=ABOUT_JSON, db_file=DB_FILE, expected_profile_url=None):
    ...
    if expected_profile_url:
        ...
        try:
            actual = normalize_instagram_target(profile_url)
        except ValueError:
            actual = profile_url.strip().rstrip('/').lower()
        try:
            expected = normalize_instagram_target(expected_profile_url)
        except ValueError:
...
def import_posts(posts_json=POSTS_JSON, db_file=DB_FILE, profile_id=None):
    """Import IG feed posts into photo_posts (scoring/face compatible)."""
    ...
    for item in items:
        photo_url = item.get("post_url") or item.get("photo_url") or item.get("url", "")
        if not photo_url:
            continue
...
def import_all(
    ...
    print("\n" + "═" * 65)
    print("BIRDY-EDWARDS LITE — Instagram DB Importer")
    print("═" * 65)
