...
from platforms.instagram.db import import_all, compute_frequency, extract_top7
from platforms.instagram.about_sb import main as scrape_about
from platforms.instagram.reels_sb import main as scrape_reels
...
def home():
    ...
    return render_template('instagram/index.html', cookie_status=cookie_status)


@instagram_bp.route('/analysis')
def analysis():
    ...
    if not profile:
        return redirect(url_for('instagram.home'))
    return render_template('instagram/analysis.html', profile=profile, profile_id=profile_id)


@instagram_bp.route('/api/import-cookies', methods=['POST'])
def import_cookies():
    """
    Receive cookie JSON from Cookie-Editor extension,
    convert to pickle format and save as ig_cookies.pkl.
    """
    ...
    try:
        ...
        if not cookies_have_domain(cookies, 'instagram.com'):
            return jsonify({
                'ok': False,
                'error': 'No Instagram cookies found — export cookies from instagram.com'
            })
        ...
        if 'sessionid' not in names:
            return jsonify({
                'ok': False,
                'error': 'Missing sessionid cookie — make sure you are logged into Instagram before exporting'
            })

        ig_cookies = filter_cookies_for_domain(cookies, 'instagram.com')
        converted = []
        for c in ig_cookies:
            selenium_cookie = {
                'name':     c.get('name', ''),
...
@instagram_bp.route('/api/check-cookies')
def check_cookies():
...
@instagram_bp.route('/api/verify-session')
def verify_session():
    """
    ...
    try:
        ...
        if not cookies_have_domain(cookies, 'instagram.com'):
            return {'exists': True, 'count': len(cookies), 'cookies': cookies,
                    'error': 'Cookie file has no Instagram domain cookies'}
...
@instagram_bp.route('/api/investigations')
...
@instagram_bp.route('/api/investigations/<int:profile_id>', methods=['DELETE'])
...
@instagram_bp.route('/api/start-pipeline', methods=['POST'])
def start_pipeline():
    ...
    try:
        profile_url = normalize_instagram_target(profile_url)
...
@instagram_bp.route('/api/pipeline-status')
def pipeline_status():
    return jsonify({
        'ok':          True,
...
def _run_pipeline(profile_url, depth):
    ...
    try:
        ...
        for p, step_id in [(p1, 'about'), (p2, 'posts'), (p3, 'reels')]:
            p.join()
            if p.exitcode == 0:
        ...
        try:
            import_all(
                about_json = os.path.join(BASE_DIR, 'ig_about.json'),
...
def _scrape_top7_about(top7, profile_id):
    ...
    for entry in top7:
        ...
        try:
            ...
            about_file = os.path.join(BASE_DIR, 'ig_about.json')
...
@instagram_bp.route('/api/profile-summary/<int:profile_id>')
...
@instagram_bp.route('/api/all-interactors/<int:profile_id>')
...
@instagram_bp.route('/api/top7/<int:profile_id>')
...
@instagram_bp.route('/api/graph-data/<int:profile_id>')
...
@instagram_bp.route('/api/cocomment-graph/<int:profile_id>')
...
@instagram_bp.route('/api/timeline/<int:profile_id>')
...
@instagram_bp.route('/api/activity-metrics/<int:profile_id>')
...
@instagram_bp.route('/api/post-type-counts/<int:profile_id>')
...
@instagram_bp.route('/api/face-clusters/<int:profile_id>')
...
@instagram_bp.route('/api/face-cluster/<int:cluster_id>/members')
...
@instagram_bp.route('/api/photo-posts/<int:profile_id>')
...
@instagram_bp.route('/api/text-posts/<int:profile_id>')
...
@instagram_bp.route('/api/reel-posts/<int:profile_id>')
